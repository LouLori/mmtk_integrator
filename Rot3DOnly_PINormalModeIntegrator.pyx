...
#
cdef class Rot3DOnly_PINormalModeIntegrator(MMTK.PIIntegratorSupport.PIIntegrator):
    ...
    cdef void atomtocm(self, N.ndarray[double, ndim=2] x, N.ndarray[double, ndim=2] v,
         ...
         for i in range (Nmol):
            ...
            for z in range (nbeadspmol):
                ...
                #print "atomtocm, mcm", mcm[i*nbeadspmol+z]
                ...
                #print "atomtocm, xcm", xcm[i*nbeadspmol+z], "for Mol", i, "bead",z
...
##### Lori Modified the eulertocart function (General for C2V triatomic molecule (FROM WFF TO SFF
    cdef void eulertocart(self, int bindex, int molind, N.ndarray[double,ndim=2] rcm_stand, N.ndarray[double, ndim=2] x, N.ndarray[double,ndim=1] eulerangles, N.ndarray[double,ndim=2] xcm):
        cdef N.ndarray[double,ndim=2] R, rcm_New
        ...
        rcm_New = N.zeros((natomspmol, 3), N.float)
        ...
        for a in range(natomspmol):
            rcm_New[a]=N.dot(R, rcm_stand[a])
            # After Rotation, the xcm doesn't change
            x[(aindex+a)*nbeadspmol+bindex]=xcm[molind*nbeadspmol+bindex]+rcm_New[a]
            #print "Mol no.", molind, "in bead", bindex, "xcm", xcm[molind*nbeadspmol+bindex]
            #print "rcm_new", rcm_New[a]
...
##### Generate Rotational Matrix (FROM MFF to SFF CM
    def matpre(self, N.ndarray[double,ndim=1] eulang):
...
##### Getting the delta Euler angles between two sets of Euler angles (in WFF)
    def deleul(self, N.ndarray[double,ndim=1] eulan1, N.ndarray[double,ndim=1] eulan2):
        ...
        # We cannot use the built-in fn N.matmul(), for it is available only for numpy 1.9 and later, but not our version
        rotma1t = self.rotinv(rotma1)
        rotmar = N.dot(rotma1t,rotma2)
...
#### Rot Density Generator
    ...
    #    # the angles of rho is based on degree 
...
#############################
#### Rot Density Generator
    def rotden(self, N.ndarray[double,ndim=1] eulan1, N.ndarray[double,ndim=1] eulan2, N.ndarray[double,ndim=1] rotmat, int istop):
        cdef double wntok, matrot
        cdef N.ndarray[double, ndim=1] eulrel
        eulrel= N.zeros((3), N.float)
        eulrel = self.deleul(eulan1, eulan2)
        # the angles of rho is based on degree 
        ...
        cdef int indchi, indphi, indthe, index_p0, index_p1
        cdef double rot_0, delchi, delphi, delthe
        # Flooring the euler angles to get the index of the angles         
        indchi=int(chi)
        indphi=int(phi)
        indthe=int(theta)
        ...
        # This index is based on chi, for each fixed theta, we have 361 phi indexes, and for each fixed phi, we have 361 chi indexes
        index_p0=(indthe*361+indphi)*361+indchi # index for point 0
        rot_0=rotmat[index_p0]

        if (indchi != 360):
            index_p1=(indthe*361+indphi)*361+indchi+1 # next index is just the difference by 1 degree
            # CHI part for each properties
            delchi=rotmat[index_p1]-rot_0

        if (indphi != 360):
            index_p1=(indthe*361+indphi+1)*361+indchi
            # PHI part for each properties
            delphi=rotmat[index_p1]-rot_0

        if (indthe != 180):
            index_p1=((indthe+1)*361+indphi)*361+indchi
            # THETA part for each properties
            delthe=rotmat[index_p1]-rot_0

        # Linear Interpolation for all three angles
        matrot=rot_0+delchi*(chi-N.float(indchi))+delphi*(phi-N.float(indphi))+delthe*(theta-N.float(indthe))
...
###############################
    ...
    cdef start(self):
        cdef double acceptratio, rd, sint, pot_old, pot_new, dens_old, dens_new, erot_old, erot_new, indexp0val, indexp1val
        cdef int na, t0b, t1b, t2b, t0, t1, t2, atombead, indexp0, indexp1, indexp0n, indexp1n
        ...
        cdef int natoms, nbeads, nsteps, step, df, cdf, nb, Nmol, Ntruemol, istop
	...
	# MATT-Introduce X-COM variable, number of molecules Nmol
        ...
        ## Construct the standard BFF of water
        ...
        rcm_standC[0] = 0.0
        rcm_standC[1] = 0.0
        rcm_standC[2] = 0.0065303176
        rcm_standR[0] = 0.075695
        rcm_standR[1] = 0.0
        rcm_standR[2] = -0.0522425412
        rcm_standL[0] = -0.075695
        rcm_standL[1] = 0.0
        rcm_standL[2] = -0.0522425412

        rcm_stand[0] = rcm_standL
        rcm_stand[1] = rcm_standR
        rcm_stand[2] = rcm_standC
        ## Calculate the xcm for water
        ...
        for i in range(Nmol):
            ...
            if (natomspmol>1):
               for p in range(nbeadspmol):
                    rcm_L[i*nbeadspmol+p]=x[(aindex+atomL)*nbeadspmol+p]-xcm[i*nbeadspmol+p]
                    rcm_R[i*nbeadspmol+p]=x[(aindex+atomR)*nbeadspmol+p]-xcm[i*nbeadspmol+p]
                    rcm_C[i*nbeadspmol+p]=x[(aindex+atomC)*nbeadspmol+p]-xcm[i*nbeadspmol+p]
                    MCAngles[:,i*nbeadspmol+p]=self.EulerGen(m[(aindex+atomL)*nbeadspmol+p], m[(aindex+atomR)*nbeadspmol+p], m[(aindex+atomC)*nbeadspmol+p], rcm_L[i*nbeadspmol+p], rcm_R[i*nbeadspmol+p], rcm_C[i*nbeadspmol+p])
        ...
        for step in range(nsteps):
            ...
            for t1b in range(P):
                ...
                for a in range(Nmol):
                    ...
                    rho = self.rotden(Eulan1, Eulan2, rhomat, istop)
                    dens_old = rho
            ...
            for a in range(Nmol):
                ...
                for t1b in range(P):
                    ...
                    erot = self.rotden(Eulan1, Eulan2, erotmat, istop)
                    #esq = self.rotden(Eulan1, Eulan2, esqmat, istop)
                    srot+=erot
            ...
            #print "eulertocart CPU time: {0:1.8f} s".format((timepot2start-timeconvertstart)/float(CLOCKS_PER_SEC))
            #print "calculate potential CPU time: {0:1.8f} s".format((timeanalysis2start-timepot2start)/float(CLOCKS_PER_SEC))
            #print "potential CPU time: {0:1.8f} s".format((timerotstart-timestepend)/float(CLOCKS_PER_SEC))
            #print "rotation eng CPU time: {0:1.8f} s".format((timerotend-timerotstart)/float(CLOCKS_PER_SEC))
            #print "one step CPU time: {0:1.8f} s".format((timestepend-timestepstart)/float(CLOCKS_PER_SEC))
            #raise()

            #calculate new xcm based on new x
            #self.atomtocm(x,v,g,m,xcm,vcm,gcm,mcm,bdcm,Nmol)
            # End of time step
            time += delta_t
            self.foldCoordinatesIntoBox()
        ...
        acceptratio/=Ntruemol*float(P)*nsteps
...
#
cdef class Rot3DOnly_PILangevinNormalModeIntegrator(Rot3DOnly_PINormalModeIntegrator):
    ...
    cdef start(self):
        ...
        Rot3DOnly_PINormalModeIntegrator.start(self)
